...
/**
 * The {@link etaCommunication} handles communication with eta
...
public class etaCommunication {
    ...
    private static final String eta_USERNAME = "admin";
    ...
    private final Logger logger = LoggerFactory.getLogger(etaCommunication.class);
    ...
    public etaCommunication(String address, int port, String password, HttpClient httpClient) {
    ...
    public static List<etaUdpResponse> autoDiscover() {
        return updBroadcast();
    }

    public etaStatusResponse commandStatus() throws etaException, IOException {
        commandLogin();

        List<String> responseList = sendGet("result.cgi?xs");

        if (responseList.isEmpty()) {
            throw new etaException("Empty Response");
        }

        etaStatusResponse response = new etaStatusResponse();
            ...
            if (m.matches()) {
                ...
                switch (command) {
                    ...
                    case "ac":
                        ...
                        break;
                    case "os":
                        response.setOperatingStatus(etaOperatingStatus.valueOf(status.toUpperCase()));
                        break;
                    case "cs":
                        response.setLastCommandStatus(etaCommandStatus.valueOf(status.toUpperCase()));
                        break;
                    case "rz":
                        response.setLastCommandResult(etaCommandResult.valueOf(status.toUpperCase()));
                        break;
                    case "ri":
    ...
    public synchronized boolean commandIrrigate(int delay, int zone1, int zone2, int zone3, int zone4, int zone5,
        ...
        try {
            ...
            logger.warn("Could not send irrigate command to eta: {}", e.getMessage());
            return false;
        }
    ...
    public synchronized boolean commandClear() {
        try {
            ...
            logger.warn("Could not send clear command to eta: {}", e.getMessage());
            return false;
        }
    ...
    public synchronized boolean commandLogin() throws etaException {
        try {
            sendGet("/ergetcfg.cgi?lu=" + eta_USERNAME + "&lp=" + password);
        } catch (IOException e) {
            logger.warn("Could not send clear command to eta: {}", e.getMessage());
            return false;
        }
    ...
    public synchronized boolean commandLogout() {
        try {
            ...
            logger.warn("Could not send logout command to eta: {}", e.getMessage());
            return false;
        }
    ...
    private synchronized List<String> sendGet(String command) throws IOException {
        ...
        try {
            ...
            if (response.getStatus() != HttpURLConnection.HTTP_OK) {
                logger.warn("eta return status other than HTTP_OK : {}", response.getStatus());
                return Collections.emptyList();
            }
        } catch (TimeoutException | ExecutionException e) {
            logger.warn("Could not connect to eta with exception: {}", e.getMessage());
            return Collections.emptyList();
        } catch (InterruptedException e) {
            logger.warn("Connect to eta interrupted: {}", e.getMessage());
    ...
    private static List<etaUdpResponse> updBroadcast() {
        List<etaUdpResponse> rList = new LinkedList<>();
        ...
        try (DatagramSocket c = new DatagramSocket()) {
            ...
            while (true) {
                ...
                // on eta
                rList.add(new etaUdpResponse(deviceTypeBC, addressBC, portBC, unqiueNameBC, ""));
            }
